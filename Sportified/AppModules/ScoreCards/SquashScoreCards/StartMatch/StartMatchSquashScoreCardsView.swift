//
//  StartMatchSquashScoreCardsView.swift
//  Sportified
//
...
struct StartMatchSquashScoreCardsView: View {
    @ObservedObject var viewModel: StartMatchSquashScoreCardsViewModel
            ...
            VStack{
                ...
                ScrollView(showsIndicators: false){
                    ...
                    VStack(alignment: .leading){
                        ...
                        HStack(spacing: 2){
                            Text("Select Points to win").font(Font.mulishBold(size: 18)).foregroundColor(Color.textColor)
                            Text("*").font(Font.mulishMedium(size: 18)).foregroundColor(Color.red)
                                .frame(maxWidth: .infinity,alignment: .leading)
                        }.padding(.top,25)
                 
                        CustomDropDown(title: viewModel.pointstoWin ?? "Points to win",
                                       isShow: $viewModel.isShowPointstowin,
                                       list: viewModel.pointstowinList,
                                       callBack: {name,index in
                            
                            viewModel.pointstowinList = viewModel.pointstowinList.map { user in
                                var updatedUser = user
                                updatedUser.isSelected = false
                                return updatedUser
                            }
                            viewModel.pointstowinList[index].isSelected = true
                            viewModel.pointstoWin = name
                            withAnimation(.easeInOut(duration: 0.3)) {
                                viewModel.isShowPointstowin.toggle()
                            }
                        })
                        HStack(spacing: 2){
                        ...
                        HStack {
                            Text("Handicap").foregroundColor(Color.textColor).font(Font.mulishMedium(size: 16))
                            Spacer()
...
#Preview {
    StartMatchSquashScoreCardsView(viewModel: StartMatchSquashScoreCardsViewModel())
}
